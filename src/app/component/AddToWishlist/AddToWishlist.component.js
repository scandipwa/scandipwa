...
import './AddToWishlist.style';
import TextPlaceholder from 'Component/TextPlaceholder';

export default class AddToWishlist extends PureComponent {
    static propTypes = {
        id: PropTypes.number,
    ...
    renderButton() {
        const { existsInWislist } = this.props;

        return (
            <button
              onClick={ this.onClick }
              title={ !existsInWislist ? __('Add to Wishlist') : __('Remove from Wishlist') }
            >
               { !existsInWislist ? this.renderPlusHeart() : this.renderMinusHeart() }
            </button>
        );
    }
