...
export default class Footer extends PureComponent {
    static propTypes = {
        ...
        footerLogoSize: PropTypes.shape({
    ...
    renderLogoImage() {
        const {
            header_logo_src, logo_alt, footerLogoSize
        } = this.props;

        if (header_logo_src) {
            return <img src={ `/media/logo/${header_logo_src}` } style={ footerLogoSize } alt={ logo_alt } />;
        }

        return (
            <svg xmlns="http://www.w3.org/2000/svg" width="70" height="10" viewBox="0 0 116 17">
    ...
    renderCopyrightContainer() {
        const { copyright, isLoading } = this.props;

        if (isLoading) return null;

        return (
            <div block="Footer" elem="CopyrightContainer">
                { this.renderLogo() }
